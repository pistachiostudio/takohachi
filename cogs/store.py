...
class Store(commands.Cog):
        ...
        async with ctx.typing():
            ...
            if len(register_info) != 0:
                ...
                try:
                    ...
                    # nm_skin_list = get_night_market(skins_data, weapons_data) これnight marketがないときにエラーになる KeyError: 'BonusStore'
    ...
    async def register(self, ctx):
        ...
        embed = discord.Embed()
        embed.description = "<:p01_pepebrim:951023068275421235>:warning: このコマンドはあなたの`Riot ID`と`Riot Password`と`Discord ID`をデータベースに保存します。\n\nデータベースは暗号化され、安全に管理されていますが、万が一流出などの事故が起きても一切の責任を負いません。"
        await ctx.reply(embed=embed)

        embed = discord.Embed()
        embed.description = "<:p01_pepechamber:951023019017527336> 上記に同意する場合のみ、まずは `Riot ID` を入力してください。"
