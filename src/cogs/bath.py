...
class Bath(commands.Cog):
    ...
    async def b(self, interaction: discord.Interaction):
        ...
        if rtn is False:
            ...
            await interaction.response.send_message(f"🛀をつけました◎", ephemeral=True, delete_after=5)
            pass

        if rtn is True:
            after_nick = before_nick.lstrip("🛀")
            await interaction.user.edit(nick=after_nick)
            await interaction.response.send_message(f"🛀をはずしました◎", ephemeral=True, delete_after=5)
            pass
