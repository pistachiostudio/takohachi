...
class Currency(commands.Cog):
    ...
    async def bonus(self, interaction: discord.Interaction):
        ...
        if len(istrue) == 0:
            ...
            # bonusが0の場合はmoneyの値にBONUS_VALUEを足して更新する
                ...
                embed.timestamp = datetime.now(JST)
                embed.color = discord.Color.green()
                embed.description = f":moneybag:<@{sender_id}>はボーナスを獲得し、所持金の合計が \
                      {update_user_money_t} pisになりました。"
                await interaction.response.send_message(embed=embed)
                db.close()
                return
    ...
    async def pay(self, interaction: discord.Interaction, amount: int, give_user: discord.Member):
        ...
        if str(command_sender_id) == str(give_user.id):
            embed = discord.Embed()
            embed.color = discord.Color.dark_green()
            embed.description = ":warning:自分に送金することはできません。"
            await interaction.response.send_message(embed=embed)
            db.close()
            return
            ...
            # お金が足りる場合
                ...
                if len(give_user_money) == 0:
                    ...
                    try:
                        sql = (
                            "insert into currency (user_id,user_name,bonus,money) values(?,?,?,?)"
                        )
                        ...
                        sender_update_tuple = (
                            int(update_sender_money),
                            str(command_sender_id),
                        )
                        sql = "update currency set money = ? where user_id = ?"
                ...
                # give_userがすでにデータベースに登録がある人の場合
                    ...
                    try:
                        ...
                        give_user_update_tuple = (
                            int(update_give_user_money),
                            str(give_user_id),
                        )
                        sql = "update currency set money = ? where user_id = ?"
    ...
    @app_commands.command(name="rich", description="お金持ちランキングを表示します。")
    async def rich(self, interaction: discord.Interaction):
        ...
        if record_len == 0:
            ...
            c.execute(query)
            richest_list = c.fetchall()
            rich_text = f":one: <@{richest_list[0][0]}> {richest_list[0][3]:,} \
            ...
            rich_text = f":one: <@{richest_list[0][0]}> {richest_list[0][3]:,} \
                pis\n:two: <@{richest_list[1][0]}> {richest_list[1][3]:,} pis\n:three: \
                <@{richest_list[2][0]}> {richest_list[2][3]:,} pis"
            embed = discord.Embed()
            embed.color = discord.Color.dark_green()
            embed.title = "おかねもちらんきんぐ"
            embed.description = rich_text
            ...
            richest_list = c.fetchall()
            rich_text = f":one: <@{richest_list[0][0]}> {richest_list[0][3]:,} pis\n\
                :two: <@{richest_list[1][0]}> {richest_list[1][3]:,} pis\n:three: <@{richest_list[2][0]}> \
                {richest_list[2][3]:,} pis\n:four: <@{richest_list[3][0]}> {richest_list[3][3]:,} pis"
            embed = discord.Embed()
            embed.color = discord.Color.dark_green()
            embed.title = "おかねもちらんきんぐ"
            embed.description = rich_text
...
# shopからアイテムを買うことができます
    ...
    buy_item: str
            ...
            if len(item_all_record) == 0:
                embed = discord.Embed()
                embed.color = discord.Color.dark_green()
                embed.description = f":warning:その商品は現在販売されていないようです。\
                    !!shopコマンドで販売中の商品を確認してください。"
                await interaction.response.send_message(embed=embed)
                return
    ...
    async def resetcurrency(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed()
        embed.color = discord.Color.dark_green()
        embed.description = "データベースのテーブル currency のデータをすべて削除しました。"
        await interaction.response.send_message(embed=embed)
        db.close()
        return
    ...
    async def setmoney(
        ...
        if bonus == "0" or bonus == "1":
            ...
            if len(istrue) == 0:
                ...
                embed.color = discord.Color.dark_green()
                embed.description = (
                    f"<@{set_user_id}> に{amount_t} pis セットしました。bonus_flagは{bonus}です。"
                )
                await interaction.response.send_message(embed=embed)
                db.close()
                return
            ...
            embed = discord.Embed()
            embed.color = discord.Color.dark_green()
            embed.description = "bonus_flagは0か1以外設定できません。"
            await interaction.response.send_message(embed=embed)
            db.close()
            return
